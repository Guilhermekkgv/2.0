    ...
    function LinuxLib.Tab(Name)
        ...
        TabBtn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                if CurrentTab then
                    ...
                    Tabs[CurrentTab].Button.TextColor3 = Linux.Theme.Text
                end
        ...
        function TabElements.Dropdown(config)
            ...
            local DropFrame = Linux.Instance("ScrollingFrame", {
                ...
                Position = UDim2.new(0, Dropdown.Position.X.Offset + 110, 0, Dropdown.Position.Y.Offset + 55),
                CanvasSize = UDim2.new(0, 0, 0, 0),
            ...
            local function UpdateDropSize()
                ...
                local tween = TweenInfo.new(0.2, Enum.EasingStyle.Quad)
                if IsOpen then
                    TweenService:Create(DropFrame, tween, {Size = UDim2.new(0, Dropdown.Size.X.Offset, 0, maxHeight)}):Play()
                    TweenService:Create(Arrow, tween, {Rotation = 180}):Play()
                else
                    TweenService:Create(DropFrame, tween, {Size = UDim2.new(0, Dropdown.Size.X.Offset, 0, 0)}):Play()
                    TweenService:Create(Arrow, tween, {Rotation = 0}):Play()
                end
                DropFrame.CanvasSize = UDim2.new(0, 0, 0, DropLayout.AbsoluteContentSize.Y)
            end
            ...
            Main.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch and IsOpen then
                    ...
                    if touch.X < pos.X or touch.X > pos.X + size.X or touch.Y < pos.Y or touch.Y > pos.Y + size.Y then
                        IsOpen = false
                        DropFrame.Visible = false
                        UpdateDropSize()
                    end
