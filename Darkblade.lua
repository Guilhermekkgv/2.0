    ...
    function LinuxLib.Tab(Name)
        ...
        function TabElements.Dropdown(config)
            ...
            local function AdjustElementsBelow()
                local siblings = TabContent:GetChildren()
                local dropIndex = 0
                for i, child in pairs(siblings) do
                ...
                for i, child in pairs(siblings) do
                    if child:IsA("Frame") or child:IsA("TextButton") then
                        if i > dropIndex then
                            local originalY = (i - 1) * (30 + 4)
                            local newY = IsOpen and originalY + dropHeight + 4 or originalY
