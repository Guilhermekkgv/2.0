...
function Linux.Create(Name)
    ...
    function LinuxLib.Tab(Name)
        ...
        function TabElements.Dropdown(config)
            local Dropdown = Linux.Instance("TextButton", { -- Mudei pra TextButton pra toque mais direto
                Parent = TabContent,
                BackgroundColor3 = Linux.Theme.Element,
                Size = UDim2.new(1, -5, 0, 30),
                LayoutOrder = config.LayoutOrder or #TabContent:GetChildren(),
            ...
            InputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch and IsOpen then
                    ...
                    if not (touch.X >= pos.X and touch.X <= pos.X + size.X and touch.Y >= pos.Y and touch.Y <= pos.Y + size.Y) then
                        IsOpen = false
                        UpdateSize()
                        AdjustElementsBelow()
                    end
